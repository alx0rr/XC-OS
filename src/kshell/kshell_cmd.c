...
void kshell_cmd_ls(int argc, char** argv) {
    const char* path = (argc > 0) ? argv[0] : NULL;
    xcfs_list(path);
}

void kshell_cmd_cd(int argc, char** argv) {
    if (argc < 1) {
        xcfs_chdir("/");
        return;
    }
...
void kshell_cmd_pwd(void) {
    printf("%s\n", xcfs_getcwd());
}

void kshell_cmd_mkdir(int argc, char** argv) {
    if (argc < 1) {
...
void kshell_cmd_cat(int argc, char** argv) {
    if (argc < 1) {
    ...
    if (!buffer) { printf("{FG(255,0,0)}cat: out of memory\n"); return; }
    int rd = xcfs_read(argv[0], buffer, size - 1);
    if (rd >= 0) {
        buffer[rd] = 0;
        printf("{FG(0,255,0)}");
        for (int i = 0; i < rd; i++) printf("%c", buffer[i]);
...
void kshell_cmd_echo(int argc, char** argv) {
    if (argc == 0) { printf("\n"); return; }
...
void kshell_cmd_touch(int argc, char** argv) {
    if (argc < 1) { printf("{FG(255,0,0)}Usage: touch <file>\n"); return; }
...
void kshell_cmd_rm(int argc, char** argv) {
    if (argc < 1) { printf("{FG(255,0,0)}Usage: rm <file>\n"); return; }
...
int kshell_cmd_cp(int argc, char** argv) {
    if (argc < 2) { printf("{FG(255,0,0)}Usage: cp <source> <dest>\n"); return -1; }
    xcfs_dirent_t info;
    if (xcfs_stat(argv[0], &info) < 0) {
    ...
    uint8_t* buffer = (uint8_t*)pmm_malloc(size);
    if (!buffer) { printf("{FG(255,0,0)}cp: out of memory\n"); return -1; }
...
void kshell_cmd_mv(int argc, char** argv) {
    if (argc < 2) { printf("{FG(255,0,0)}Usage: mv <source> <dest>\n"); return; }
    if (kshell_cmd_cp(argc, argv) != 0) return;
    if (xcfs_delete(argv[0]) != 0)
        printf("{FG(255,165,0)}Warning: could not remove source file\n");
}

void kshell_cmd_stat(int argc, char** argv) {
    if (argc < 1) { printf("{FG(255,0,0)}Usage: stat <file>\n"); return; }
    xcfs_dirent_t info;
    if (xcfs_stat(argv[0], &info) < 0) {
        printf("{FG(255,0,0)}stat: %s: no such file or directory\n", argv[0]);
        return;
    }
    const char* tstr = (info.type == 0x02) ? "directory" : "file";
    printf("{FG(255,255,0)}File:  %s\n", argv[0]);
    printf("{FG(255,255,255)}Type:  %s\n", tstr);
    printf("Size:  %u bytes\n", info.size);
    printf("Flags: %s%s%s\n",
           (info.flags & 0x04) ? "[protected] " : "",
           (info.flags & 0x08) ? "[executable] " : "",
           (info.flags & 0x10) ? "[readonly]" : "");
}
...
void kshell_cmd_tree(int argc, char** argv) {
...
void kshell_cmd_uname(void) {
    printf("XC-OS v0.6 i386 (XCFS v2)\n");
}

void kshell_cmd_free(void) {
    pmm_print_stats();
}

void kshell_cmd_uptime(void) {
...
void kshell_cmd_memtest(void) {
    ...
    for (uint32_t i = 0; i < alloc_count; i++) {
        ...
        for (uint32_t j = 0; j < 1024; j++) ptr[j] = (uint8_t)(j & 0xFF);
    ...
    for (uint32_t i = 0; i < alloc_count; i++) {
        ...
        for (uint32_t j = 0; j < 1024; j++)
            if (ptr[j] != (uint8_t)(j & 0xFF)) block_errors++;
        errors += block_errors;
        if (block_errors == 0)
...
void kshell_cmd_bench(void) {
    ...
    for (uint32_t i = 0; i < 10000000; i++) sum += i;
    ...
    if (mem) {
        ...
        for (uint32_t i = 0; i < 256 * 1024; i++) ptr[i] = i;
...
void kshell_cmd_banner(int argc, char** argv) {
    if (argc == 0) { printf("{FG(255,0,0)}Usage: banner <text>\n"); return; }
    char text[256] = {0};
    for (int i = 0; i < argc; i++) {
        strcat(text, argv[i]);
...
void kshell_cmd_sleep(int argc, char** argv) {
    if (argc < 1) { printf("{FG(255,0,0)}Usage: sleep <milliseconds>\n"); return; }
    u32 ms = 0;
    for (int i = 0; argv[0][i]; i++) {
        if (argv[0][i] >= '0' && argv[0][i] <= '9')
            ms = ms * 10 + (argv[0][i] - '0');
        else { printf("{FG(255,0,0)}Invalid number\n"); return; }
    }
...
void kshell_cmd_countdown(int argc, char** argv) {
    if (argc < 1) { printf("{FG(255,0,0)}Usage: countdown <seconds>\n"); return; }
    u32 seconds = 0;
    for (int i = 0; argv[0][i]; i++) {
        if (argv[0][i] >= '0' && argv[0][i] <= '9')
    ...
    if (seconds > 60) { printf("{FG(255,0,0)}Maximum 60 seconds\n"); return; }
...
void kshell_cmd_pitbench(void) {
    ...
    printf("{FG(255,255,255)}Accuracy:       {FG(0,255,0)}%u.%02u%%\n\n",
           (elapsed * 100) / 1000, ((elapsed * 10000) / 1000) % 100);
    ...
    printf("Actual ticks:   {FG(255,255,0)}%u\n", elapsed);
    printf("{FG(255,255,255)}Accuracy:       {FG(0,255,0)}%u.%02u%%\n\n",
           (elapsed * 100) / 5000, ((elapsed * 10000) / 5000) % 100);
}

void kshell_cmd_beep(int argc, char** argv) {
    if (argc < 2) { printf("{FG(255,0,0)}Usage: beep <frequency> <duration_ms>\n"); return; }
    u32 frequency = 0;
    for (int i = 0; argv[0][i]; i++) {
        if (argv[0][i] >= '0' && argv[0][i] <= '9')
            frequency = frequency * 10 + (argv[0][i] - '0');
        else { printf("{FG(255,0,0)}Invalid frequency\n"); return; }
    }
    u32 duration = 0;
    for (int i = 0; argv[1][i]; i++) {
        if (argv[1][i] >= '0' && argv[1][i] <= '9')
            duration = duration * 10 + (argv[1][i] - '0');
        else { printf("{FG(255,0,0)}Invalid duration\n"); return; }
    }
