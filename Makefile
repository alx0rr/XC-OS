...
build: $(CFG)
	...
	@gcc $(CFLAGS) -c $(DRIVERS)/net/ne2000.c           -o $(BUILD)/ne2000.o      || { echo -e "$(RED)✗$(RESET) Failed: ne2000.c"      >&2; exit 1; }
	@gcc $(CFLAGS) -c $(DRIVERS)/net/eth.c              -o $(BUILD)/eth.o         || { echo -e "$(RED)✗$(RESET) Failed: eth.c"         >&2; exit 1; }
	@gcc $(CFLAGS) -c $(DRIVERS)/net/arp.c              -o $(BUILD)/arp.o         || { echo -e "$(RED)✗$(RESET) Failed: arp.c"         >&2; exit 1; }
	@gcc $(CFLAGS) -c $(DRIVERS)/net/ip.c               -o $(BUILD)/ip.o          || { echo -e "$(RED)✗$(RESET) Failed: ip.c"          >&2; exit 1; }
	@gcc $(CFLAGS) -c $(DRIVERS)/net/icmp.c             -o $(BUILD)/icmp.o        || { echo -e "$(RED)✗$(RESET) Failed: icmp.c"        >&2; exit 1; }
	@gcc $(CFLAGS) -c $(DRIVERS)/net/udp.c              -o $(BUILD)/udp.o         || { echo -e "$(RED)✗$(RESET) Failed: udp.c"         >&2; exit 1; }
	@gcc $(CFLAGS) -c $(DRIVERS)/net/dns.c              -o $(BUILD)/dns.o         || { echo -e "$(RED)✗$(RESET) Failed: dns.c"         >&2; exit 1; }
	@gcc $(CFLAGS) -c $(DRIVERS)/net/tcp.c              -o $(BUILD)/tcp.o         || { echo -e "$(RED)✗$(RESET) Failed: tcp.c"         >&2; exit 1; }
	@gcc $(CFLAGS) -c $(DRIVERS)/net/http.c             -o $(BUILD)/http.o        || { echo -e "$(RED)✗$(RESET) Failed: http.c"        >&2; exit 1; }
