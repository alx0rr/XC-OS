...
static int xcfs_check_bounds(uint32_t start_sector, uint32_t size) {
    if (size == 0) return -1;
    uint32_t sectors = (size + XCFS_SECTOR_SIZE - 1) / XCFS_SECTOR_SIZE;
    uint32_t last_sector = start_sector + sectors - 1;
    uint32_t fs_end = XCFS_START_SECTOR + xcfs_header.total_sectors;
    if (last_sector >= fs_end) return -1;
    return 0;
}
...
void xcfs_init(uint8_t drive) {
    ...
    for (uint32_t i = 0; i < header_sectors; i++) {
        ...
        uint32_t copy_bytes = entries_per_sector * sizeof(xcfs_entry_t);
        if (copy_bytes > XCFS_SECTOR_SIZE) copy_bytes = XCFS_SECTOR_SIZE;
        memcpy(&xcfs_entries[i * entries_per_sector], xcfs_sector_buf, copy_bytes);
    }
...
int xcfs_create(const char* name, uint32_t size) {
    ...
    uint32_t copy_bytes = entries_per_sector * sizeof(xcfs_entry_t);
    if (copy_bytes > XCFS_SECTOR_SIZE) copy_bytes = XCFS_SECTOR_SIZE;
    memcpy(hdr_buf, &xcfs_entries[entry_sector * entries_per_sector], copy_bytes);
    ata_write_sector(xcfs_ctx.drive, XCFS_START_SECTOR + 1 + entry_sector, hdr_buf);
    return 0;
}
    ...
    for (uint32_t i = 0; i < xcfs_header.file_count; i++) {
        if (strcmp(xcfs_entries[i].name, name) == 0) {
            ...
            for (uint32_t s = 0; s < header_sectors; s++) {
                ...
                uint32_t copy_bytes = entries_per_sector * sizeof(xcfs_entry_t);
                if (copy_bytes > XCFS_SECTOR_SIZE) copy_bytes = XCFS_SECTOR_SIZE;
                memcpy(buf, &xcfs_entries[s * entries_per_sector], copy_bytes);
                ata_write_sector(xcfs_ctx.drive, XCFS_START_SECTOR + 1 + s, buf);
            }
            if (header_sectors < (XCFS_DATA_START - (XCFS_START_SECTOR + 1))) {
                uint8_t zero_buf[XCFS_SECTOR_SIZE];
                memset(zero_buf, 0, XCFS_SECTOR_SIZE);
                ata_write_sector(xcfs_ctx.drive, XCFS_START_SECTOR + 1 + header_sectors, zero_buf);
            }
            return 0;
        }
    }
    ...
    for (uint32_t i = 0; i < xcfs_header.file_count; i++) {
        if (strcmp(xcfs_entries[i].name, name) == 0) {
            ...
            if (size > entry->size) {
                ...
                uint32_t old_sectors = (entry->size + XCFS_SECTOR_SIZE - 1) / XCFS_SECTOR_SIZE;
                ...
                for (uint32_t s = 0; s < old_sectors; s++) {
                    uint8_t zero_buf[XCFS_SECTOR_SIZE];
                    memset(zero_buf, 0, XCFS_SECTOR_SIZE);
                    ata_write_sector(xcfs_ctx.drive, old_start + s, zero_buf);
                }
            }
            uint32_t sectors = (size + XCFS_SECTOR_SIZE - 1) / XCFS_SECTOR_SIZE;
            ...
            uint32_t copy_bytes = entries_per_sector * sizeof(xcfs_entry_t);
            if (copy_bytes > XCFS_SECTOR_SIZE) copy_bytes = XCFS_SECTOR_SIZE;
            memcpy(hdr_buf, &xcfs_entries[sector_index * entries_per_sector], copy_bytes);
