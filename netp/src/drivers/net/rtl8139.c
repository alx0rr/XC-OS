...
static void rtl8139_setup_buffers(void) {
    rtl8139_dev.rx_buffer = (uint8_t*)pmm_malloc(RTL8139_RX_BUF_SIZE + 2048);
    if (!rtl8139_dev.rx_buffer) {
        printf("{FG(255,0,0)}RTL8139: Failed to allocate RX buffer\n");
        return;
    }
    rtl8139_dev.rx_buffer_phys = (uint32_t)rtl8139_dev.rx_buffer;
    memset(rtl8139_dev.rx_buffer, 0, RTL8139_RX_BUF_SIZE + 2048);
    for (int i = 0; i < RTL8139_NUM_TX_DESC; i++) {
        rtl8139_dev.tx_buffer[i] = (uint8_t*)pmm_malloc(RTL8139_TX_BUF_SIZE);
        if (!rtl8139_dev.tx_buffer[i]) {
            printf("{FG(255,0,0)}RTL8139: Failed to allocate TX buffer %d\n", i);
            return;
        }
        rtl8139_dev.tx_buffer_phys[i] = (uint32_t)rtl8139_dev.tx_buffer[i];
        memset(rtl8139_dev.tx_buffer[i], 0, RTL8139_TX_BUF_SIZE);
    }
